	...
	$(document).ready(function() {
		...
		function tampildata() {
			$('#showTable').DataTable({
				...
				"ajax": {
					url: "<?php echo base_url() ?>pakar/getdata",
					type: "POST",
					data: function(d) {
		...
		$('#showTable').on('click', '.bhapus', function() {
			var id = $(this).attr('data');
			swal.fire({
				title: 'Yakin Menghapus data pakar ini?',
				...
				if (result.value) {
					$.ajax({
						url: '<?= base_url('pakar/hapus_pakar') ?>',
						type: 'POST',
						dataType: 'json',
						data: {
							id: id
