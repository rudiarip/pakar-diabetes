	...
	$(document).ready(function() {
		...
		function tampildata() {
			$('#showTable').DataTable({
				...
				"ajax": {
					url: "<?php echo base_url() ?>usermanagement/getdata",
					type: "POST",
					data: function(d) {
		...
		$('#inputform').submit(function(e) {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url('usermanagement/store') ?>",
		...
		$('#showTable').on('click', '.bedit', function() {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url('usermanagement/showedit') ?>",
				dataType: "JSON",
				data: {
					id: id
				},
				dataType: 'JSON',
				beforeSend: function() {
					$('#crudLabel').text('Edit User Admin')
					$('#akun').hide();
		...
		$('#showTable').on('click', '.bhapus', function() {
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						url: '<?= base_url('usermanagement/destroy') ?>',
						type: 'POST',
						dataType: 'json',
						data: {
							id: id
		...
		tambahData = () => {
			$('#crudLabel').text('Tambah User Admin')
			$('.form-input').val('');
			$('#crudModal').modal('show');
			$('#akun').show();
			$('#status').val('Y');
