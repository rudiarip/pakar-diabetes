...
if (!function_exists('rangeArray')) {
    ...
    * 	> angka pertamanya
    * @param int $interval
    * 	> interval angkanya pas jadi array
    *
    * @return array
    */
    function rangeArray(int $first, int $last, int $interval = null)
    {
        if (is_null($interval)) return range($first, $last);
        return range($first, $last, $interval);
    }
}

if (!function_exists('createArrayYears')) {
    /**
    * bikin array tahun dari tahun $firstYear sampe tahun $capYears
    *
    * @param int $firstYear
    * 	> tahun awalnya
    * 		ex. 2020
    * @param int|null $capYears
    * 	> tahun sekarang ditambah berapa tahun ke depan
    * 		ex. 4, berarti 2021 + 4 = tahun 2025
    *
    * @return array
    */
    function createArrayYears(int $firstYear, int $capYears = null)
    {
        $cap = date('Y');
        if ($capYears) {
            $now = new \DateTime();
            $res = $now->add(new \DateInterval('P' . $capYears . 'Y'));
            $cap = $res->format('Y');
        }

        return rangeArray((int) $cap, $firstYear);
    }
}
